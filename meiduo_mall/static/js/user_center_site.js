var vm = new Vue({
    ...
    methods: {
        ...
        save_address(){
            ...
            if (this.error_receiver || this.error_place || this.error_mobile || this.error_email || !this.form_address.province_id || !this.form_address.city_id || !this.form_address.district_id) {
                ...
                if (this.editing_address_index === '') {
                    // 新增地址
                    var url = this.host + '/address/create/';
                    ...
                    // 修改地址
                    var url = this.host + '/address/' + this.addresses[this.editing_address_index].id + '/';
        ...
        // 设置默认地址
        set_default(index){
            var url = this.host + '/address/' + this.addresses[index].id + '/default/';
            axios.put(url, {}, {
                headers: {
                    'X-CSRFToken': getCookie('csrftoken')
                },
        ...
        save_title(index){
            if (!this.input_title) {
                ...
                var url = this.host + '/address/' + this.addresses[index].id + '/title/';
                axios.put(url, {
